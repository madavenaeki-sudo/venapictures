...
const Leads: React.FC<LeadsProps> = ({
    ...
    const [editedLeadWhatsapp, setEditedLeadWhatsapp] = useState('');
    ...
    const parsePhoneNumberFromNotes = (notes: string | undefined): string | null => {
    ...
    const getPhoneNumberForLead = (lead: Lead): string | null => {
        if (lead.whatsapp) {
            return cleanPhoneNumber(lead.whatsapp);
        }
        return parsePhoneNumberFromNotes(lead.notes); // Fallback to old method
    };
    ...
    const handleSendBookingForm = (lead: Lead) => {
        const phoneNumber = getPhoneNumberForLead(lead);

        if (!phoneNumber) {
            showNotification('Nomor WhatsApp tidak ditemukan. Buka detail untuk follow up manual.');
        ...
        showNotification('Membuka WhatsApp untuk mengirim form booking.');
    };
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>, newStatus: LeadStatus) => {
        ...
        if (lead && lead.status !== newStatus) {
            if (newStatus === LeadStatus.CONVERTED) {
                ...
                setConversionForm(prev => ({...prev, projectName: `Proyek ${lead.name}`, location: lead.location, projectType: userProfile.projectTypes[0], whatsapp: lead.whatsapp || ''}));
            } else {
    ...
    const handleConvertLead = (e: React.FormEvent) => {
        ...
        const newClient: Client = {
            ...
            whatsapp: conversionForm.whatsapp,
        ...
        if (newProject.amountPaid > 0) {
            const newTransaction: Transaction = {
                ...
                method: 'Transfer Bank', cardId: conversionForm.dpDestinationCardId,
            };
    ...
    const handleUpdateLead = () => {
        ...
        const updatedLead = { ...selectedLead, notes: editedLeadNotes, whatsapp: editedLeadWhatsapp };
    ...
    const handleTriggerConversion = () => {
        ...
        setConversionForm(prev => ({ ...prev, projectName: `Proyek ${selectedLead.name}`, location: selectedLead.location, whatsapp: selectedLead.whatsapp || '' }));
        handleCloseLeadDetail();
    };

    const handleShareFollowUp = () => {
        if (!selectedLead) return;
        const phoneNumber = getPhoneNumberForLead(selectedLead);
        if (phoneNumber) {
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button
                                                    onClick={(e) => {
                                                        ...
                                                        showNotification('Tautan formulir booking disalin!');
                                                    }}
                                                    className="text-xs font-semibold text-brand-text-secondary hover:text-brand-accent hover:underline inline-flex items-center gap-1"
                                                    title="Salin Link Form Booking"
                                                >
                                                    Salin Link <Share2Icon className="w-3 h-3"/>
                                                </button>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleSendBookingForm(lead); }}
                                                    className="text-xs font-semibold text-brand-accent hover:underline inline-flex items-center gap-1"
                                                    title="Kirim Form Booking via WhatsApp"
                                                >
                                                    Kirim <SendIcon className="w-4 h-4"/>
                                                </button>
                                            </div>
                                        )}
                ...
                <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Tambah Prospek Manual">
                    <form onSubmit={handleAddLeadSubmit} className="space-y-4">
                        ...
                        <div className="input-group">
                         ...
                         <div className="input-group">
                            <input type="tel" id="newLeadWhatsapp" name="whatsapp" value={newLeadForm.whatsapp} onChange={handleNewLeadFormChange} className="input-field" placeholder=" " />
                            <label htmlFor="newLeadWhatsapp" className="input-label">Nomor WhatsApp (Opsional)</label>
                        </div>
                        <div className="input-group">
                            ...
                            <label htmlFor="newLeadNotes" className="input-label">Catatan</label>
                        </div>
                ...
                <Modal isOpen={isDetailModalOpen} onClose={handleCloseLeadDetail} title={`Detail Prospek: ${selectedLead.name}`}>
                    {isEditingLead ? (
                        <div className="space-y-4">
                            <div className="input-group">
                                <input type="text" id="leadWhatsapp" value={editedLeadWhatsapp} onChange={(e) => setEditedLeadWhatsapp(e.target.value)} className="input-field" placeholder=" " />
                                <label htmlFor="leadWhatsapp" className="input-label">Nomor WhatsApp</label>
                            </div>
                            <div className="input-group">
                                <textarea id="leadNotes" rows={5} value={editedLeadNotes} onChange={(e) => setEditedLeadNotes(e.target.value)} className="input-field" placeholder=" " />
                        ...
                        <div className="space-y-4">
                            ...
                            <p><strong>WhatsApp:</strong> {selectedLead.whatsapp ? <a href={`https://wa.me/${cleanPhoneNumber(selectedLead.whatsapp)}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{selectedLead.whatsapp}</a> : 'Tidak ada'}</p>
                            ...
                            <div className="flex justify-between items-center pt-6 border-t border-brand-border">
                                ...
                                {selectedLead.status === LeadStatus.DISCUSSION && (
                                    <button 
                                        type="button" 
                                        onClick={() => {
                                            handleSendBookingForm(selectedLead);
                                            handleCloseLeadDetail();
                                        }} 
                                        className="button-primary inline-flex items-center gap-2"
                                    >
                                        <SendIcon className="w-4 h-4" /> Kirim Form Booking
                                    </button>
                                )}
                                {selectedLead.status === LeadStatus.FOLLOW_UP && (
