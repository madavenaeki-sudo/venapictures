...
import { PencilIcon, Trash2Icon, PlusIcon, Share2Icon, FileTextIcon } from '../constants';
...
const Packages: React.FC<PackagesProps> = ({ packages, setPackages, addOns, setAddOns, projects }) => {
  ...
  const publicPackagesUrl = useMemo(() => {
    return `${window.location.origin}${window.location.pathname}#/public-packages`;
  }, []);

  useEffect(() => {
    if (isShareModalOpen) {
        const qrCodeContainer = document.getElementById('packages-page-qrcode');
        if (qrCodeContainer && typeof (window as any).QRCode !== 'undefined') {
            qrCodeContainer.innerHTML = '';
            new (window as any).QRCode(qrCodeContainer, {
                text: publicPackagesUrl,
                width: 200,
                height: 200,
                colorDark: "#020617",
                colorLight: "#ffffff",
  ...
  }, [isShareModalOpen, publicPackagesUrl]);
  ...
  const downloadPackagesQrCode = () => {
    const canvas = document.querySelector('#packages-page-qrcode canvas') as HTMLCanvasElement;
    if (canvas) {
        const link = document.createElement('a');
        link.download = 'halaman-paket-layanan-qr.png';
  ...
  return (
    <div>
      ...
      <Modal isOpen={isShareModalOpen} onClose={() => setIsShareModalOpen(false)} title="Bagikan Halaman Paket Layanan" size="sm">
          <div className="text-center p-4">
              <div id="packages-page-qrcode" className="p-4 bg-white rounded-lg inline-block mx-auto"></div>
              <p className="text-xs text-brand-text-secondary mt-4 break-all">{publicPackagesUrl}</p>
              <div className="flex items-center gap-2 mt-6">
                  <button onClick={copyPackagesLinkToClipboard} className="button-secondary w-full">Salin Tautan</button>
                  <button onClick={downloadPackagesQrCode} className="button-primary w-full">Unduh QR</button>
              </div>
          </div>
      </Modal>
    </div>
