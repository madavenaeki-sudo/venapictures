                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-2">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div className="md:col-span-2 pt-6 border-t border-gray-700/50">
                                <h4 className="font-semibold text-gradient mb-4">Progres Sub-Status</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <div className="flex justify-between items-center mb-2">
                                            <label className="text-sm font-medium text-brand-text-secondary">Daftar Sub-Status Proyek</label>
                                            <button type="button" onClick={addCustomSubStatus} className="text-xs font-semibold text-brand-accent hover:underline">+ Tambah</button>
                                        </div>
                                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 bg-brand-bg p-2 rounded-lg border border-brand-border">
                                            {(formData.customSubStatuses || []).map((sub, index) => (
                                                <div key={index} className="p-2 bg-brand-input rounded-md grid grid-cols-1 gap-2">
                                                    <div className="flex items-center gap-2">
                                                        <input
                                                            type="text"
                                                            value={sub.name}
                                                            onChange={e => handleCustomSubStatusChange(index, 'name', e.target.value)}
                                                            placeholder="Nama Sub-Status"
                                                            className="input-field !p-1.5 !text-sm flex-grow !mt-0"
                                                        />
                                                        <button type="button" onClick={() => removeCustomSubStatus(index)} className="p-1 text-brand-danger hover:bg-brand-danger/10 rounded-full flex-shrink-0">
                                                    ...
                                                    <input
                                                        type="text"
                                                        value={sub.note}
                                                        onChange={e => handleCustomSubStatusChange(index, 'note', e.target.value)}
                                                        placeholder="Catatan/Deskripsi"
                                                        className="input-field !p-1.5 !text-xs !mt-0"
                                                    />
                                                </div>
                                            ))}
                                            {(formData.customSubStatuses || []).length === 0 && <p className="text-xs text-center text-brand-text-secondary py-4">Belum ada sub-status.</p>}
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium text-brand-text-secondary mb-2 block">Pilih Sub-Status Aktif</label>
                                        <div className="p-3 border border-brand-border bg-brand-bg rounded-lg space-y-2 max-h-48 overflow-y-auto">
                                            {(formData.customSubStatuses || []).filter(s => s.name.trim() !== '').map(opt => (
                                            ...
                                            {(formData.customSubStatuses || []).filter(s => s.name.trim() !== '').length === 0 && <p className="text-xs text-center text-brand-text-secondary py-4">Buat daftar sub-status terlebih dahulu.</p>}
                                        </div>
                                    </div>
                                </div>
                            </div>
